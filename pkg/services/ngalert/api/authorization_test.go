	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			t.Run("should fail with insufficient permissions", func(t *testing.T) {
				...
				var permissionCombinations [][]actionscope
				for bit := 0; bit < l*l; bit++ {
					...
					permissionCombinations = append(permissionCombinations, tuple)
				}

				for _, perms := range permissionCombinations {
					if len(perms) == len(flattenPermissions) {
						continue
					}
					missing := make(map[string][]string)
					for _, perm := range perms {
						missing[perm.action] = append(missing[perm.action], perm.scope)
					}
					fmt.Println(missing)
					executed := false
					err := authorizeRuleChanges(groupChanges, func(evaluator ac.Evaluator) bool {
						response, err := evaluator.Evaluate(missing)
						require.NoError(t, err)
						executed = true
						return response
					})
