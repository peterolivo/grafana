...
func (st DBstore) GetAlertRulesGroupByRuleUID(ctx context.Context, query *ngmodels.GetAlertRulesGroupByRuleUIDQuery) error {
	return st.SQLStore.WithDbSession(ctx, func(sess *sqlstore.DBSession) error {
		...
		err := sess.Table("alert_rule").Alias("A").Select("A.*").Join(
			"INNER",
			sess.Table("alert_rule").Alias("B"), "A.org_id = B.org_id AND A.namespace_uid = B.namespace_uid AND A.rule_group = B.rule_group AND B.uid = ?", query.UID,
		).Where("org_id = ?", query.OrgID).Find(&result)
		if err != nil {
			return err
		}
		query.Result = result
